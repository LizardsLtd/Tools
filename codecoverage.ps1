function runOpenCover([string]$csproj){
    ...
    .\OpenCover.4.6.519\tools\OpenCover.Console.exe -target:"c:\Program Files\dotnet\dotnet.exe" -targetargs:"test -c Release $csproj" -mergeoutput -hideskipped:File -output:coverage.xml -oldStyle -register:user -filter:"-[Picums.Tests.*]* +[Picums*]*" -searchdirs:test/$fileName/bin/Release/netcoreapp2.0
}
